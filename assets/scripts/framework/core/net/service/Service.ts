...
import { Message, MessageHeader, IMessage } from "../message/Message";
...
export class Service extends ServerConnector {
    ...
    protected onMessage(data: Uint8Array) {
        ...
        log(`recv data main cmd : ${header.mainCmd} sub cmd : ${header.subCmd} buffer length : ${header.size}`);
        let key = makeKey(header.mainCmd, header.subCmd);

        if (!this._listeners[key]) {
            warn(`no find listener data main cmd : ${header.mainCmd} sub cmd : ${header.subCmd}`);
            return;
        }
        ...
        if (this._messageHeader) {
            if (msg.encode()) {
                ...
                if (this.isHeartBeat(msg)) {
                    if (DEBUG) log(`send request main cmd : ${msg.mainCmd} , sub cmd : ${msg.subCmd} `);
                } else {
                    log(`send request main cmd : ${msg.mainCmd} , sub cmd : ${msg.subCmd} `);
                }
