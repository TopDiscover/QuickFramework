                                ...
                                if (json) {
                                    me._loadRemoteRes(spineAtlas, JsonAsset, isNeedCache).then((atlas: TextAsset) => {
                                        if (atlas) {
                                            ...
                                            cache = cache as td.Resource.CacheData;
                                            cache.info.url = url;
                                            asset.name = url;
                                            cache.data = asset;
                                            cache.isLoaded = true;
...
export class _AssetManager {
    ...
    public load(
        ...
        if (cache) {
            ...
            if (cache.isLoaded) {
                //已经加载完成
                if (DEBUG && cache.status == td.Resource.CacheStatus.WAITTING_FOR_RELEASE) {
    ...
    private _onLoadComplete(cache: td.Resource.CacheData, completeCallback: (data: td.Resource.CacheData) => void, err: Error | null, data: Asset | Asset[] | null) {
        cache.isLoaded = true;
        //添加引用关系
        let tempCache = cache;
        if (err) {
