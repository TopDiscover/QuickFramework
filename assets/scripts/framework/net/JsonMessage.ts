...
export class JsonMessage extends Message {
    ...
    private serialize(): any {
        ...
        for (let len = serializeKeyList.length, i = 0; i < len; i++) {
            ...
            if (null === serializeObj) {
                warn("Invalid serialize member : " + memberName);
            }
    ...
    private serializeMap(value: Map<any, any>) {
        ...
        value.forEach((value, key) => {
            ...
            if (null === serVal.k) {
                warn("Invalid map key!");
                serVal.k = '';
            }
            if (null === serVal.v) {
                warn("Invalid map value");
                serVal.v = '';
            }
            result.push(serVal);
        });
    ...
    private deserialize(data: any) {
        ...
        for (let len = serializeKeyList.length, i = 0; i < len; i++) {
            ...
            if (!iscomplete) {
                warn("Invalid deserialize member :" + memberName);
                return false;
            }
        }
        return true;
                ...
                switch (memberType) {
                    ...
                    default: {
                        ...
                        if ((<any>this)[memberName] instanceof JsonMessage) {
                            ...
                            warn("Invalid deserialize member :" + memberName + " value:" + originValue);
                            return false;
                        }
                    } break;
                }
