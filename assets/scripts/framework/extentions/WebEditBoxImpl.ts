...
class WebEditBoxHelper {
    ...
    init() {
        ...
        if (this.input == null) {
            ...
            (<any>input.style)['-moz-appearance'] = "textfield";

            game.container?.appendChild(input);
            this.input = input;
        }
        ...
        if (this.textarea == null) {
            ...
            game.container?.appendChild(textarea);
    ...
    private adjust(rotate: number | string) {
        if (rotate == 0 || rotate == 180) {
            let height = parseInt((<any>game.container).style.height);
            let rate = 0.90;
...
export default class WebEditBoxImpl {
    ...
    init(delegate: EditBox) {
        ...
        if (delegate.inputMode === EditBox.InputMode.ANY) {
    ...
    private _updateInputType() {
        ...
        if (this._isTextArea) {
            ...
            else if (inputFlag === EditBox.InputFlag.INITIAL_CAPS_WORD) {
        ...
        if (inputMode === EditBox.InputMode.EMAIL_ADDR) {
            type = 'email';
        } else if (inputMode === EditBox.InputMode.NUMERIC || inputMode === EditBox.InputMode.DECIMAL) {
            type = 'number';
        } else if (inputMode === EditBox.InputMode.PHONE_NUMBER) {
            type = 'number';
            (<any>elem).pattern = '[0-9]*';
        } else if (inputMode === EditBox.InputMode.URL) {
            type = 'url';
        } else {
            type = 'text';

            if (returnType === EditBox.KeyboardReturnType.SEARCH) {
                type = 'search';
            }
        }
        (<any>elem).type = type;
