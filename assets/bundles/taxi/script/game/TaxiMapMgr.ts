...
import { TaxiResUtil } from "../data/TaxiResUtil";
...
export class TaxiMapMgr extends Component {
    ...
    public resetMap(){
        ...
        const currMap = this._currMap.getComponent(TaxiMap) as TaxiMap;
    ...
    private _buildModel(type: string){
        ...
        TaxiResUtil.getMapObjs(type, arrName, undefined, (err: any, arrPrefabs: Prefab[])=>{
            if (err) {
                console.error(err);
                return;
            }
            ...
            for (let idx = 0; idx < objPlane.children.length; idx++) {
                ...
                let node = TaxiPoolManager.getNode(prefab, nodeParent);
    ...
    private _createRoadPoint(objPoint: any, parent: Node){
        ...
        let point = nodeRoadPoint.addComponent(TaxiRoadPoint);
        ...
        if (point.type === TaxiRoadPoint.RoadPointType.GOODBYE) {
            this.maxProgress++;
        }
    ...
    private _triggerFinished(type: string){
        ...
        if (tips) {
            dispatch('updateLoading', 10, tips);
        }
