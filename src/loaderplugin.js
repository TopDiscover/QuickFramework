...
function downloadFile(url, onComplete) {
    //web
    var xhr = new XMLHttpRequest(), errInfo = 'loaderplugin failed: ' + url + ', status: ';
    xhr.open("GET", url, true);
    xhr.onload = function () {
        if (xhr.status === 200 || xhr.status === 0) {
            onComplete && onComplete(null, xhr.responseText);
        } else {
 ...
 * 加载图片
 */
function downloadImage(item, options, callback) {
    downloadImages(item, function (err, data) {
        let img = new Image();
        if (!err && typeof data === 'string') {
            img.src = data.substring(0, data.length - 10);
 ...
 * 加载图片
 *
 * @param {string}  item 可以是图片地址，也开始是图片的Base64编码
 * @param {Function} onComplete  回调函数
 */
function downloadImages(item, onComplete) {
    var url = item;
    //原生平台
    if (CC_JSB) {
        ...
        if (typeof result === "string" && result) {
            ...
            onComplete && onComplete(new Error("Download text failed: " + url), null);;
        }
        return;
    }
    //远程
    if (url.startsWith("http")) {
        onComplete && onComplete(null, url);
        return;
    }
    downloadFile(url, onComplete);
}
 ...
 * 加载txt文本
 */
function downloadText(item, options, callback) {
    var url = item;
    //原生平台
    if (CC_JSB) {
        var result = jsb.fileUtils.getStringFromFile(url);
        ...
        if (typeof result === "string" && result) {
            onComplete && onComplete(null, Base64.decode(result));
        } else {
            onComplete && onComplete(new Error("Download text failed: " + url), null);;
        }
        return;
    ...
    downloadFile(url, function (err, data) {
        var text = "";
        if (!err && typeof data === 'string') {
            text = data.substring(0, data.length - 10);
            callback(null, Base64.decode(text));
        } else {
            callback(err, text);
        }
    });
 ...
 * 加载json文本
 */
function downloadJson(item, options, callback) {
    var url = item;
    //原生平台
    if (CC_JSB) {
        var result = jsb.fileUtils.getStringFromFile(url);
        ...
        if (typeof result === "string" && result) {
            onComplete && onComplete(null, JSON.parse(data));
        } else {
            onComplete && onComplete(new Error("Download text failed: " + url), null);;
        }
        return;
    ...
    downloadFile(url, function (err, data) {
        var json = "";
        if (!err && typeof data === 'string') {
            json = data.substring(0, data.length - 10);
