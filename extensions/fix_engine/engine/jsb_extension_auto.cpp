...
static bool js_extension_Manifest_constructor(se::State& s) // NOLINT(readability-identifier-naming) constructor_overloaded.c
{
    ...
    do {
        if (argc == 3) {
            ...
            ok &= sevalue_to_native(args[2], &arg2, s.thisObject());
            if (!ok) { ok = true; break; }
            cc::extension::Manifest* cobj = JSB_ALLOC(cc::extension::Manifest, arg0.value(), arg1.value(),arg2.value());
            s.thisObject()->setPrivateData(cobj);
            return true;
        }
    } while(false);
...
static bool js_extension_AssetsManagerEx_setHotUpdateUrl(se::State& s) // NOLINT(readability-identifier-naming)
{
    CC_UNUSED bool ok = true;
    auto* cobj = SE_THIS_OBJECT<cc::extension::AssetsManagerEx>(s);
    ...
    do {
        if (argc == 1) {
            ...
            ok &= sevalue_to_native(args[0], &arg0, s.thisObject());
            if (!ok) { ok = true; break; }
            cobj->setHotUpdateUrl(arg0.value());
