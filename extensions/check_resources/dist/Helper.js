...
class Helper {
    ...
    logWarn(res, url) {
        console.warn(`资源${res.path} => ${res.compPath}.${res.type} 引用了不属于自己游戏的资源 ${url}`);
    }
    onCheckSubGame(gameName) {
        let otherGamesTexture = [];
        console.log(`正在获取其它游戏资源列表`);
        this.getOtherGameResources(gameName, otherGamesTexture, (resources) => {
            let gamePrefabs = [];
            console.log(`获取其它游戏资源列表完成`);
            console.log(`正在获取${this.bundles[gameName].db}预置体资源`);
            this.getGamePrefabs(gameName, gamePrefabs, (results) => {
                console.log(`获取${this.bundles[gameName].db}预置体资源成功`);
                results.forEach((result) => {
                    let viewData = fs_1.readFileSync(result.path);
                    //取出该界面引用的资源
                    console.log(`正在检测${result.url}`);
                            ...
                            case "cc.Animation":
                                {
                                    ...
                                    if (!res.defaultClip && !res.clips) {
                                        console.warn(`资源${res.path} => ${res.compPath}.${res.type} 只添加了动画组件，但没有任何动画`);
                                    }
                                }
                                break;
                        }
                    }
                    console.log(`检测${result.url}完成!!`);
                });
            });
        });
    }
