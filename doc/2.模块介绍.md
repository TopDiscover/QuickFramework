...
### 网络辅助模块
- 网络请求辅助模块(NetHelper)**该模块只是提示一个统一的请求接口，当你需要把游戏每成模块化进行管理，
可继承实现达到模块化，在外部直接使用，如果是公共类请求接口，如登录，聊天，公告等游戏公共模块请求接口
建议直接放置到Manager中，方便其它模块调用，但如果是bundle特定模块，请勿添加到Manager中，保持Bundle
的独立性**
- 参考示例：
```ts
//Vip模块
...
```
### 网络控制器管理
- 该管理器主要对各模块的网络控制器模块进行统一集中化管理，包含添加,删除,注册等操作。
**管理器可实现模块化管理，如把所有大厅的放在一个管理器中，所有全局需要用到的网络
控制器放另一个管理器中，在指定的地方做统一的添加及删除操作**
- 如项目中的
```ts
...
export default class TestChatNetController extends Controller<ChatService> {
    protected addEvents() {
        super.addEvents()
        this.addNetEvent(GetCmdKey(MainCmd.CMD_LOBBY, SUB_CMD_LOBBY.Chat), this.onRecvChat, ChatInfo, true);
    }
    private onRecvChat(data: ChatInfo) {
        if ( Manager.gameView ){
            Manager.gameView.showChat(data);
        }
    }
}
Manager.hallNetManager.register(TestChatNetController);
