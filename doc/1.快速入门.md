...
[LoginLogic](https://gitee.com/top-discover/QuickFramework/blob/3.1.0/assets/scripts/login/LoginLogic.ts#L29)打开登录界面，
可在主包的登录界面中设置一个按钮，进入GameOneView界面

- 参考代码

```ts
private onClick(ev: cc.Event.EventTouch) {
   Manager.bundleManager.enterBundle(new HotUpdate.BundleConfig(Manager.getLanguage("hall_view_game_name.0", HallData.bundle), "gameOne", 1));
}
```
**到此，整个小白教程完成，如何创建自己的界面及bundle，更新详细的部分请参考[模块介绍](https://gitee.com/top-discover/QuickFramework/blob/3.1.0/doc/2.模块介绍.md)**